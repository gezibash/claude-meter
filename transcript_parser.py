...
BUILD_PATTERNS = [
    'go build', 'npm run build', 'npm build', 'yarn build',
    'cargo build', 'make', 'cmake', 'tsc', 'webpack', 'vite build',
    'docker build', 'gradle build', 'mvn compile', 'mvn package'
]
TEST_PATTERNS = [
    'go test', 'npm test', 'npm run test', 'yarn test',
    'pytest', 'python -m pytest', 'jest', 'vitest',
    'cargo test', 'make test', 'mocha', 'rspec', 'gradle test', 'mvn test'
]
...
class TranscriptParser:
    ...
    def _compute_stride(self) -> dict:
        ...
        recent = self.stride_window[-window:] if len(self.stride_window) > window else self.stride_window
    ...
    def _compute_focus(self) -> dict:
        ...
        def get_state(win: list) -> tuple[str, str]:
            ...
            top_dir = max(dir_counts.items(), key=lambda x: x[1]) if dir_counts else ('', 0)
            top_file = max(file_counts.items(), key=lambda x: x[1]) if file_counts else ('', 0)
        ...
        # Top edit file
        top_edit = max(self.edits.items(), key=lambda x: x[1]) if self.edits else ('', 0)
